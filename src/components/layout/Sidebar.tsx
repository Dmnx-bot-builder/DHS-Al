import { LayoutDashboard, CandlestickChart, Wallet, History, Brain, Newspaper, Settings, LifeBuoy, TrendingUp, LineChart, Zap, FlaskConical, X } from 'lucide-react';
...
export function Sidebar({ active, onSelect, collapsed, mobileOpen, onCloseMobile }: { active: string; onSelect: (id: string) => void; collapsed: boolean; mobileOpen: boolean; onCloseMobile: () => void; }) {
  ...
  const renderItems = (items: NavItem[]) => items.map((item) => {
    ...
    return (
      <button key={item.id} onClick={() => handleSelect(item.id)}
        className={`group relative flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all duration-200 ${isActive ? 'bg-brand-500/10 text-white' : 'text-slate-400 hover:bg-white/5 hover:text-slate-200'} ${collapsed && !mobileOpen ? 'justify-center' : ''}`}
        ...
        {collapsed && !mobileOpen && item.badge && <span className="absolute right-1.5 top-1.5 h-2 w-2 rounded-full bg-bull-400" />}
      </button>
    );
  });
  ...
  return (
    <>
      ...
      <aside className={`fixed left-0 top-0 z-50 flex h-screen flex-col border-r border-white/[0.06] bg-ink-900/95 backdrop-blur-xl transition-all duration-300 ${
        mobileOpen ? 'w-64 translate-x-0' : 'w-64 -translate-x-full lg:translate-x-0'
      } ${collapsed && !mobileOpen ? 'lg:w-[72px]' : 'lg:w-64'}`}>
        <button onClick={onCloseMobile} className="absolute right-3 top-4 flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 hover:bg-white/5 hover:text-white lg:hidden">
          <X className="h-5 w-5" />
        </button>

        <div className={`flex h-16 items-center border-b border-white/[0.06] ${collapsed && !mobileOpen ? 'justify-center px-2 lg:px-2' : 'px-5'}`}>
          <div className="flex items-center gap-3">
            ...
            {(!collapsed || mobileOpen) && <div className="leading-tight"><p className="text-sm font-bold tracking-tight text-white">DHS AI</p><p className="text-[10px] tracking-wide text-slate-500">Hybrid Scalping</p></div>}
          </div>
        </div>
        <nav className="no-scrollbar flex-1 space-y-1 overflow-y-auto p-3">
          {(!collapsed || mobileOpen) && <p className="px-3 pb-1 pt-3 text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-600">Trading</p>}
          {renderItems(mainNav)}
          {(!collapsed || mobileOpen) && <p className="px-3 pb-1 pt-5 text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-600">System</p>}
          {renderItems(utilNav)}
        </nav>
