...
function App() {
  ...
  return (
    <div className="relative min-h-screen bg-ink-950 text-slate-200">
      ...
      <div className={`relative transition-all duration-300 ${collapsed ? 'ml-[72px]' : 'ml-64'}`}>
        <TopBar onToggleSidebar={() => setCollapsed(c => !c)} />
        <main className="mx-auto max-w-[1600px] space-y-4 p-4 lg:p-6">
          {renderPage()}
          <footer className="mt-8 flex items-center justify-between py-4 text-[11px] text-slate-600">
